...
JSON.parse(localStorage.getItem("todos")):[])
    ...
    const deteteItems = (id) => {

        setTodoList((pvsTodos) => {
            return pvsTodos.filter((todo) => todo.id !== id)
        })
    }

    const toggle = (id) => {
        setTodoList((pvstodo) => {
            return pvstodo.map((todo) => {
                if (todo.id === id) {
    ...
    return (
       ...
       flex flex-col p-7 min-h-[550px] rounded-xl'>
         ...
         flex-1 h-14 pl-6 pr-2 placeholder:text-slate-600'
                    ...
                    bg-orange-600 w-32 h-14 text-white text-lg font-medium cursor-pointer'> ADD +
                    </button>
                </div>
                {/* todo list */}
                <div>
                    {todoList.map((item, index) => {
                        return <TodoItems key={index} text={item.text}
                            id={item.id} isComplete={item.isComplete} deteteItems={deteteItems}
                            toggle={toggle} />
                    })}
